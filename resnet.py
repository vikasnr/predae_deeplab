...
class ResNet_Bottleneck_OS16(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 50:
            ...
            resnet.load_state_dict(torch.load("D:/Altran/Edge/towards corner case/deeplabv3/pretrained_models/resnet/resnet50-19c8e357.pth"))
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("D:/Altran/Edge/towards corner case/deeplabv3/pretrained_models/resnet/resnet101-5d3b4d8f.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("D:/Altran/Edge/towards corner case/deeplabv3/pretrained_models/resnet/resnet152-b121ed2d.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
...
class ResNet_BasicBlock_OS16(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 18:
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("D:/Altran/Edge/towards corner case/deeplabv3/pretrained_models/resnet/resnet18-5c106cde.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("D:/Altran/Edge/towards corner case/deeplabv3/pretrained_models/resnet/resnet34-333f7ec4.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
...
class WideResNet_BasicBlock_OS8(nn.Module):
    def __init__(self, num_layers):
        super(WideResNet_BasicBlock_OS8, self).__init__()
        if num_layers == 50:
            resnet = models.wide_resnet50_2()
            # load pretrained model:
            resnet.load_state_dict(torch.load("D:/Altran/Edge/towards corner case/deeplabv3/pretrained_models/resnet/wide_resnet50_2-95faca4d.pth"))
            # remove fully connected layer, avg pool, layer4 and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-4])
...
# def WideResNet():
#     return torch.hub.load('pytorch/vision:v0.9.0', 'wide_resnet50_2', pretrained=True)

# def WideResNet():
#     return WideResNet_BasicBlock_OS8(num_layers=50)
# def WideResNet_9():
...
#IPdb [15]: list(resnet_18.children())
# [Conv2d(3, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False), BatchNorm2d(64, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True), ReLU(inplace=True), MaxPool2d(kernel_size=3, stride=2, padding=1, dilation=1, ceil_mode=False), Sequential(
#   (0): BasicBlock(
