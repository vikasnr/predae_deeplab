...
# val_dirs = ["frankfurt/"]
# val_dirs = ["stuttgart_00/"]
val_dirs = ["munster/", "lindau/"]
...
class DatasetVal(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # normalize the img (with the mean and std for the pretrained ResNet):
        img = img/255.0
        img = img - np.array([0.485, 0.456, 0.406]) #mean
        img = img/np.array([0.229, 0.224, 0.225]) # (shape: (512, 1024, 3)) #std dev
...
class DatasetVal_2(torch.utils.data.Dataset):
    def __init__(self,image_path,frame_id):
        # self.img_dir = cityscapes_data_path + "/leftImg8bit/predicted_frame/"
        # self.label_dir = cityscapes_meta_path + "/label_imgs/"
        ...
        example = {}
        example["img_path"] = image_path
        # example["label_img_path"] = label_img_path
        example["img_id"] = frame_id
        self.examples.append(example)
